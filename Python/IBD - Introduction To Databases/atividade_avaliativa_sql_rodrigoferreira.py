# -*- coding: utf-8 -*-
"""Atividade_avaliativa_SQL_RodrigoFerreira.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1dt8YiHxs_Y4u5KpGaK7Fw_9ssA2IluZA
...
query = """
SELECT 
  C.CPF as CPF, C.NomeCon as Nome, C.UFNascCon as estado_de_nascimento
FROM 
  Consultor as C
WHERE
...
query = """
SELECT DISTINCT
  V.Destino as Destino, L.UFLoc as uf
FROM 
  Consultor as C, Viagem as V, Local as L
WHERE
  C.CPF = V.CPF AND V.Destino = L.NomeLoc AND V.Sigla = 'RecSys'
"""
...
SELECT DISTINCT
  P.Sigla, P.NomePro
FROM
  Projeto as P NATURAL JOIN Equipe as E NATURAL JOIN Consultor as C
WHERE
  P.Sigla NOT IN (SELECT P.Sigla 
                  FROM Projeto as P NATURAL JOIN Equipe as E NATURAL JOIN Consultor as C 
...
query = """
SELECT DISTINCT * 
FROM Consultor as C INNER JOIN Viagem as V INNER JOIN Local as L
  ON C.CPF = V.CPF AND ((V.Destino = L.NomeLoc AND C.UFNascCon = L.UFLoc) OR (V.Origem = L.NomeLoc AND C.UFNascCon = L.UFLoc))
ORDER BY
  C.NomeCon
"""
...
query = """
SELECT L.NomeLoc
FROM Local as L LEFT JOIN Viagem as V ON (L.NomeLoc = V.Destino)
...
query = """
SELECT DISTINCT P.Sigla, P.NomePro
FROM Projeto as P INNER JOIN Viagem as V INNER JOIN Local as L 
  ON P.Sigla = V.Sigla AND V.Destino = L.NomeLoc
WHERE L.UFLoc = 'CA' AND V.DataViagem >= 2019
"""
...
query = """
SELECT DISTINCT C.NomeCon, E.Sigla
FROM Consultor as C INNER JOIN Equipe as E
  ON C.CPF = E.CPF 
"""
...
query = """
SELECT C.CPF, C.NomeCon
FROM Consultor as C JOIN Viagem as V
  ON C.CPF = V.CPF
WHERE (C.CPF, V.Sigla) NOT IN (SELECT
                               CPF, Sigla
                               FROM Equipe as E)
"""
...
query = """
SELECT DISTINCT C.CPF, C.NomeCOn
FROM Consultor as C JOIN Equipe as E JOIN Projeto as P
  ON C.CPF = E.CPF AND E.Sigla = P.Sigla
WHERE P.DataInicio < '2019' OR P.DataInicio > '2019'
"""
