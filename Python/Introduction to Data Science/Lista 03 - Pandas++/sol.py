...
Original file is located at
    https://colab.research.google.com/drive/16fwg1PFHuZVJ6xTbRi1yJi1ZJ2I7TzJU
...
def delay(df) -> tuple:
    ...
    return (df_new['DepDelay'].mean(), df_new['ArrDelay'].mean())
...
def high_delay1(df):
    df_new = drop_missing(df)
    not_cancelled = df_new[df_new['Cancelled'] == 0].copy()

    not_cancelled['FlightTime'] = np.zeros(not_cancelled.shape[0])

    same_day = not_cancelled.CRSArrTime >= not_cancelled.CRSDepTime
    not_cancelled.loc[same_day, 'FlightTime'] = not_cancelled.CRSArrTime - not_cancelled.CRSDepTime
    not_cancelled.loc[~same_day, 'FlightTime'] = 2400 - not_cancelled.CRSDepTime + not_cancelled.CRSArrTime

    not_cancelled = not_cancelled[['OriginAirportName', 'DestAirportName', 'FlightTime']]

    routes_grouped = (not_cancelled.groupby(['OriginAirportName', 'DestAirportName'])
    .mean()
    .sort_values(by='FlightTime', ascending=False)
    .iloc[0]
    .name)

    print(not_cancelled.groupby(['OriginAirportName', 'DestAirportName'])
    .mean()
    .sort_values(by='FlightTime', ascending=False))
...
def time_to_min(time):
  ...
  if(len(time) < 3):
    ...
    total = int(time[0:2]) * 60 + int(time[2:4])
  else:
    total = int(time[0]) * 60 + int(time[1:3])
...
def high_delay(df):
    df_new = drop_missing(df)
    not_cancelled = df_new[df_new['Cancelled'] == 0]
    not_cancelled = drop_missing(not_cancelled)

    not_cancelled['CRSDepTimeMin'] = not_cancelled['CRSDepTime']
    not_cancelled['CRSDepTimeMin'] = not_cancelled['CRSDepTimeMin'].apply(time_to_min)
    not_cancelled['CRSArrTimeMin'] = not_cancelled['CRSArrTime']
    not_cancelled['CRSArrTimeMin'] = not_cancelled['CRSArrTimeMin'].apply(time_to_min)
    not_cancelled['FlightTime'] = not_cancelled['CRSArrTimeMin'] - not_cancelled['CRSDepTimeMin']

    not_cancelled = not_cancelled[['OriginAirportName', 'DestAirportName', 'FlightTime']]
    not_cancelled['FlightTime'] = not_cancelled['FlightTime'].apply(next_day)

    routes_grouped = (
        not_cancelled.groupby(['OriginAirportName', 'DestAirportName'])
        .mean()
        .sort_values(by='FlightTime', ascending=False)
    )
    
    routes_grouped = routes_grouped.reset_index()
    (OriginAirportName, DestinationAirportName) = routes_grouped.loc[0, 'OriginAirportName'], routes_grouped.loc[0, 'DestAirportName']
...
plt.scatter(not_cancelled.DepDelay, not_cancelled.ArrDelay, edgecolor='k', alpha=0.75)
plt.xlabel('DepDelay')
plt.ylabel('ArrDelay')