...
def time_to_min(time):
  ...
  if(len(time) < 3):
    ...
    # print(time, time[0:2], time[2:4])
    total = int(time[0:2]) * 60 + int(time[2:4])
  else:
    # print(time, time[0], time[1:3])
...
def high_delay(df):
    ...
    not_cancelled['DepMin'] = not_cancelled['CRSDepTimeMin'] + not_cancelled['DepDelay']
    not_cancelled['ArrMin'] = not_cancelled['CRSArrTimeMin'] + not_cancelled['ArrDelay']
    not_cancelled['FlightTime'] = not_cancelled['ArrMin'] - not_cancelled['DepMin']

    routes_grouped = (
        not_cancelled.groupby(['OriginAirportName', 'DestAirportName'])
        [['OriginAirportName', 'DestAirportName', 'FlightTime']]
        .mean()
