...
Original file is located at
    https://colab.research.google.com/drive/1_jA4COobFxPL6N3PUfmXkcoWJ5-DuR2M
...
alpha = 1
beta = 0.5
x = df['midparentHeight'].copy()
y = df['childHeight'].copy()
def mean_error(alpha, beta, x, y):
  # SEU CODIGO AQUI
  err = 0
  den = 0
  for i in range(len(x)):
    err += error(alpha, beta, x.iloc[i], y.iloc[i])
    den += 1
  return err/den
...
y = df['childHeight'].copy()
plt.scatter(x, y, alpha=0.8, edgecolors='k', s=80)
plt.plot(x,beta*x+alpha+erro_medio1, color='magenta')
plt.xlabel('Altura média dos pais')
plt.ylabel('Altura dos filhos')
plt.title('Regressão linear simples')
despine()
...
plt.scatter(x, y, alpha=0.8, edgecolors='k', s=80)
plt.plot(x,beta*x+alpha+erro_medio2, color='magenta')
plt.xlabel('Altura média dos pais')
plt.ylabel('Altura dos filhos')
plt.title('Regressão linear simples')
despine()
...
plt.scatter(x, y, alpha=0.8, edgecolors='k', s=80)
plt.plot(x,beta*x+alpha, color='magenta')
plt.xlabel('Altura média dos pais')
plt.ylabel('Altura dos filhos')
plt.title('Regressão linear simples')
despine()
...
def r_squared(alpha, beta, x, y):
  ...
  for i in range(len(y)):
    num += (error(alpha, beta, x[i], y[i])) ** 2
    den += (y[i] - np.mean(y)) ** 2
  return 1 - (num/den)
