...
Original file is located at
    https://colab.research.google.com/drive/17dPzGeb8zpW_h31spKC8w4SgbGycvjNn
...
def sum_of_squared_errors(alpha, beta, x, y):
    ...
    for i in range(len(x)):
      sum += (error(alpha, beta, x[i], y[i])) ** 2
    return sum
    
def total_sum_of_squares(y):
    return np.std(y, ddof=1)*(len(y)-1)
...
def linear_regression(x, y):
  ...
  den = 0
  for i in range(len(x)):
    num += (x.iloc[i] - x.mean()) * (y.iloc[i] - y.mean())
    den += (x.iloc[i] - x.mean()) ** 2
  beta = num/den
...
def error_verification(alpha, beta, x, y):
  ...
  lt = 0
  for i in range(len(x)):
    if (beta * x.iloc[i] + alpha) > y.iloc[i]: gt += 1
    elif (beta * x.iloc[i] + alpha) < y.iloc[i]: lt += 1
  return (lt, gt)
