...
Original file is located at
    https://colab.research.google.com/drive/11WGlsXLXMKxBEyCNnsA4pQguPcP4Ut6i
...
def ic(data):
  ...
  low = data.mean() - (1.96 * SE)
  high = data.mean() + (1.96 * SE)
  return low, high
...
def bootstrap_mean(X):
  ...
  for i in range(n_sub):
    sample = df.sample(15, replace=True, random_state=i) 
    values[i] = sample.mean()
  
  # Cálculo dos percentis segundo o IC requerido
  alpha = 0.95
  I = (100 - (100*alpha))/2
  S = 100 - I
...
def t_obs(data):
  ...
  filter = data['floors'] == (1.0 or 1.5)
  df1 = data[filter]
  df2 = data[~filter]
  mean_1 = df1['price'].mean()
  mean_not_1 = df2['price'].mean()
  return mean_1 - mean_not_1
...
def shuffling(data):
  ...
  for i in range(N):
    ...
    diff = data[~filter]['price'].mean() - data[filter]['price'].mean()
    diffs[i] = diff
