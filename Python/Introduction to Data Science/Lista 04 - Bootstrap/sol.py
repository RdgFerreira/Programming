...
Original file is located at
    https://colab.research.google.com/drive/1TiNjwv5qpAorXpyHblop0nm0AHySOu5Q
...
def CI(array):
  ...
  var_estimador = (array.var(ddof=1)) / (array.shape[0])
  SE = np.sqrt(var_estimador)
  LI = array.mean() - (2.576 * SE) # z-score para 99% de confiança
  LS = array.mean() + (2.576 * SE)
  intervalo_de_confianca = (LI, LS)
...
x = [i for i in range_float(50.0,100.0,0.0001)]
y = ecdf(x)
#print(y)
_ = plt.plot(x, y, lw = 2)
_ = plt.xlabel('Taxa de aprovação', size = 14)
_ = plt.ylabel('(%) do total', size = 14)
plt.show()
...
def resposta():
  ...
  # A opção 4 é a melhor opção para se construir um boostrap, pois temos o menor volume de dados
  # dentre as 6 combinações e, desse modo, por meio das muitas amostras realizadas pelo bootstrap,
  # conseguiremos um intervalo de confiança razoável (apesar de termos 22 < 30 valores para instâncias municipais) de maneira eficiente, pois são poucos dados.
  # YOUR CODE HERE
  return 4
...
def bootstrap_mean(n_sub, alpha):
  ...
  df = data[data['DEPENDENCIA_ADMINISTRATIVA'] == 'Federal']
  for i in range(n_sub):
    sample = df.sample(len(df), replace=True, random_state=i) 
    values[i] = sample['NOTA_MEDIA_ESCOLA'].mean()
  
  # Cálculo dos percentis segundo os alphas fornecidos
  I = (100 - (100*alpha))/2
  S = 100 - I
  LI = np.percentile(values, I)
  LS = np.percentile(values, S)
  return values, (LI, LS)
...
def ci_amplitude():
  ...
  for k in deps:
    df = data[data['DEPENDENCIA_ADMINISTRATIVA'] == k]
    values = np.zeros(n_sub)
    ...
    LI = np.percentile(values, 2.5)
    LS = np.percentile(values, 97.5)
    if (LS - LI) > amplitude_do_ic:
      amplitude_do_ic = LS - LI
      dependencia_administrativa = k
