...
Original file is located at
    https://colab.research.google.com/drive/1MwLliKT7VC0YHDxD9s1QWbpl857TOnvD
...
def gradient(x_i, y_i, alpha, beta):
  ...
  da = (-2) * (y_i - beta*x_i - alpha)
  db = (-2) * (y_i - beta*x_i - alpha) * x_i
  return np.array([da.mean(), db.mean()])
...
def descent(x, y, param, lambda_=0.01, niter=1000):
  # YOUR CODE HERE
  # x,y : dados
  ...
  for i in range(niter):
    alpha_aux = alpha
    beta_aux = beta
    alpha = alpha_aux - (lambda_ * gradient(x, y, alpha_aux, beta_aux)[0])
    beta = beta_aux - (lambda_ * gradient(x, y, alpha_aux, beta_aux)[1])
  return [alpha, beta]
...
def descent_vec(x, y, param, lambda_=0.01, niter=1000):
  # x,y : dados
  # param : lista com valores iniciais para alpha e beta
  # lambda_ : taxa de aprendizado
  # niter : número de iterações do gradiente descendente
  ...
  for i in range(niter):
    vec_aux = vec
    vec = vec_aux - lambda_ * (gradient(x, y, vec_aux[0], vec_aux[1]))
  return vec
